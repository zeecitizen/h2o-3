...
public class AstISax extends AstPrimitive {
  ...
  public Val apply(Env env, Env.StackHelp stk, AstRoot asts[]) {
    ...
    if (breaks != null) t = new AstISax.HistTask(breaks, -1, -1/*ignored if _h==-1*/).doAll(vec);
      ...
      t = new AstISax.HistTask(computeCuts(vec, numBreaks), h, x0).doAll(vec);
    } else {
      h = (x1 - x0) / numBreaks;
      t = new AstISax.HistTask(computeCuts(vec, numBreaks), h, x0).doAll(vec);
    }
  ...
  public static double third_moment(Vec v) {
    ...
    AstISax.ThirdMomTask t = new AstISax.ThirdMomTask(mean).doAll(v);
    double m2 = t._ss / v.length();
    double m3 = t._sc / v.length();
    return m3 / Math.pow(m2, 1.5);
  }

  public static class ThirdMomTask extends MRTask<AstISax.ThirdMomTask> {
    double _ss;
    double _sc;
    final double _mean;
    ...
    @Override
    public void reduce(AstISax.ThirdMomTask t) {
      _ss += t._ss;
      _sc += t._sc;
    }
  }
  ...
  public static double fourth_moment(Vec v) {
    final double mean = v.mean();
    AstISax.FourthMomTask t = new AstISax.FourthMomTask(mean).doAll(v);
    double m2 = t._ss / v.length();
    double m4 = t._sc / v.length();
    return m4 / Math.pow(m2, 2.0);
  }

  public static class FourthMomTask extends MRTask<AstISax.FourthMomTask> {
    double _ss;
    double _sc;
    final double _mean;
    ...
    @Override
    public void reduce(AstISax.FourthMomTask t) {
      _ss += t._ss;
      _sc += t._sc;
    }
  }
  ...
  public static class HistTask extends MRTask<AstISax.HistTask> {
    ...
    @Override
    public void reduce(AstISax.HistTask t) {
