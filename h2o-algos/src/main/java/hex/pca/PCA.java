...
/*
    long mem_usage =
            _parms._pca_method == PCAParameters.Method.GramSVD ? (long)(hb._cpus_allowed * p*p * 8*/
/*doubles*//*
 * Math.log((double)_train.lastVec().nChunks())/Math.log(2.)) : 1; //one gram per core
*/
    long mem_usage =
            _parms._pca_method == PCAParameters.Method.GramSVD ? (long)(hb._cpus_allowed * p*_parms._k * 8/*doubles*/ * Math.log((double)_train.lastVec().nChunks())/Math.log(2.)) : 1; //one gram per core
