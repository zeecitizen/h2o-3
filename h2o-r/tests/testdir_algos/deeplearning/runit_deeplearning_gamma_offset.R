...
test <- function(h) {
	...
	print(min(ph[,1]))
	print(max(ph[,1]))
	expect_equal(8.303062, mean_deviance, tolerance=1e-5)
	expect_equal(51.41081, mean(ph[,1]), tolerance=1e-5 )
	expect_equal(0.9569283, min(ph[,1]), tolerance=1e-5 )
	expect_equal(428.49, max(ph[,1]), tolerance=1e-5 )
