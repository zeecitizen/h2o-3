...
test <- function(h) {
	...
	expect_equal( fit2$train.error[600], hh@model$training_metrics@metrics$MSE,tolerance=0.06)	#used to pass with 1e-6
	expect_equal(mean(pr), mean(ph[,1]),tolerance=1e-5 )
	expect_equal(min(pr), min(ph[,1]) ,tolerance=0.57)	# used to pass with 1e-3
	expect_equal(max(pr), max(ph[,1]) ,tolerance=0.56)	# used to pass with 1e-3
