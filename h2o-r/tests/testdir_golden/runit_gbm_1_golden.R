...
f_model = fith2o@model
f_model_history = fith2o@model$scoring_history

#Reported MSE from H2O through R
err <- as.data.frame(fith2o@model$scoring_history$training_rmse)    # training_MSE field was replaced with training_lmse
REPMSE <- err[4,]
...
diffsq <- diff^2
EXPMSE <- sqrt(mean(diffsq))

Log.info("Print model MSE... \n")
Log.info(paste("Length of H2O MSE Vec: ", length(fith2o@model$scoring_history$training_rmse), "\t\t", "Expected Length   : ", 4))
